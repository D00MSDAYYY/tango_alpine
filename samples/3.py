...
class HistoryPlot(QWidget):
    def __init__(self):
        ...
        self.canvas = scene.SceneCanvas(
            ...
            bgcolor="#111111",
            size=(1100, 620),
            title="Vispy combined history demo",
        )
    ...
    def _redraw(self):
        ...
        current_points = self._window_points(
            raw,
            now - self.period_sec,
            now,
            y_offset=0.0,
        ...
        for index, line in enumerate(self.background_lines, start=1):
            ...
            y_offset = -PERIOD_LANE_HEIGHT * index if self.is_paused else 0.0
            line.set_data(self._window_points(raw, window_start, window_end, y_offset))

        if self.is_paused:
            self.title.text = (
                f"Пауза: период {self.period_sec:.0f} c. "
                "Старые периоды листаются вниз как пленка."
            )
        else:
            self.title.text = (
                f"Период: {self.period_sec:.0f} c. "
                "Старые периоды показаны на фоне текущего."
            )
...
class DemoWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Vispy combined history demo")
